...
import { createBooking, deleteBooking, toggleLockSlot, updateServiceMessage, createUser } from "@/actions/bookingActions";
import { signOut } from "next-auth/react";
import { ChevronLeft, ChevronRight, Lock, LogOut, Trash2, Flame, ShieldAlert, CheckCircle2, UserPlus } from "lucide-react";
  ...
  const handleCreateUser = () => {
    startTransition(async () => {
      try {
        ...
        alert(`Brugeren '${newUsername}' blev oprettet med succes!`);
        setNewUsername("");
        setNewPassword("");
      } catch (e: any) {
        alert(e.message);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-orange-500/30">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* Her er fejlen rettet: !isAdmin er fjernet, så alle kan se beskeden hvis den er aktiv */}
        {serviceMessage?.isActive && (
        ...
        {isAdmin && (
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 shadow-xl space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="space-y-3 bg-zinc-950 p-4 rounded-2xl border border-zinc-800/50">
                <label className="text-sm text-zinc-400 flex items-center gap-2">
                  <UserPlus className="h-4 w-4" /> Opret ny bruger
                </label>
                <div className="flex flex-col gap-2">
                  <input 
                    ...
                    className="bg-zinc-900 border border-zinc-700 rounded-xl p-2.5 text-sm text-white focus:ring-1 focus:ring-orange-500 focus:outline-none"
                  />
                  <input 
                    type="password" 
                  ...
                  <button 
                    ...
                    className="bg-zinc-100 text-zinc-900 py-2.5 rounded-xl text-sm font-medium hover:bg-white transition-colors disabled:opacity-50"
                  >
                    Opret Bruger
                  </button>
                </div>
              ...
              <div className="space-y-3 bg-zinc-950 p-4 rounded-2xl border border-zinc-800/50">
                ...
                <select 
                  className="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-3 text-white focus:ring-1 focus:ring-orange-500 focus:outline-none"
              ...
              <div className="space-y-3 bg-zinc-950 p-4 rounded-2xl border border-zinc-800/50">
                ...
                <div className="flex flex-col gap-2">
                  <input 
                    type="text" 
                    className="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-2.5 text-sm text-white focus:ring-1 focus:ring-orange-500 focus:outline-none"
                    placeholder="Saunaen er under ombygning..."
                    value={msgText}
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={`flex-1 py-2.5 rounded-xl text-sm border transition-all ${msgActive ? 'bg-orange-500/20 text-orange-400 border-orange-500/30' : 'bg-zinc-800 text-zinc-400 border-zinc-700'}`}
                    >
                      {msgActive ? 'Aktiv' : 'Skjult'}
                    </button>
                    <button onClick={handleSaveMessage} className="flex-1 bg-zinc-100 text-zinc-900 py-2.5 rounded-xl text-sm font-medium hover:bg-white transition-colors">Gem besked</button>
                  </div>
                </div>
              </div>
